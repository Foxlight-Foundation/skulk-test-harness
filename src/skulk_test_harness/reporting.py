...
def _fresh_install_markdown(report: FreshInstallQualificationReport) -> str:
    ...
    if report.dashboard_audio is not None:
        ...
        lines.append(
            ...
            f"voice/language `{audio.synthesis_request_voice}/"
            f"{audio.synthesis_request_language}`, sentence requests "
            f"`{audio.multi_sentence_request_count}`, pinned "
            f"`{audio.multi_sentence_voice_pinned}`, "
            f"replay requests `{audio.replay_request_count}`, replay "
            f"voice/language/inputs "
            f"`{audio.replay_voice_pinned}/{audio.replay_language_matched}/"
            f"{audio.replay_inputs_matched}`, "
